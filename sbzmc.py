...
def main(linkpath):
    ...
    tmplist.append("SBZMC/")
    for i in tmplist:
        if (linkpath.startswith(i)):
            title = "No Open The File for settings."
            filename = "No Open The File for settings."
            openfile = ["別のファイルを探して見てください。", "", "Error: No Open The File for ignore settings."]
